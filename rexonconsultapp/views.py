...
def home(request):
	return render(request, 'rexonconsultapp/index.html')


def plan(request):
	return render(request, 'rexonconsultapp/plan.html')

def otherpayment(request):
    return render(request, 'rexonconsultapp/otherpayment.html')


def about(request):
	return render(request, 'rexonconsultapp/company.html')


def career(request):
	return render(request, 'rexonconsultapp/career.html')
...
def signin(request):
	if request.user.is_authenticated:
		...
		if request.method == "POST":
			...
			if user is not None:
				...
				print(email)
				template= render_to_string('rexonconsultapp/loginAlert2.html', {'name':username})
	...
	context={}
	return render(request, 'rexonconsultapp/login.html')


def privacy(request):
	return render(request, 'rexonconsultapp/privacy.html')
...
def main_view(request, *args, **kwargs):
	...
	return render(request, 'rexonconsultapp/main.html')
...
def signup(request):
	...
	if form.is_valid():
		if client_id is not None:
			...
			referral_template= render_to_string('rexonconsultapp/referalsignupmail.html', {'name':recommended_by_client_name, 'refereed':username})
		...
		template= render_to_string('rexonconsultapp/WelcomeEmail2.html', {'name':username,'password':password})
		plain_message= strip_tags(template)
		email_message= EmailMultiAlternatives(
		...
		second_template= render_to_string('rexonconsultapp/securityEmail.html', {'name': username, 'password_reminder':password_reminder, 'password_reminder_two':password_reminder_two})
		...
		return redirect('dashboard')
	context={'form':form}
	return render(request, 'rexonconsultapp/register.html', context)
...
def dashboard(request):
	...
	return render(request, 'rexonconsultapp/dashboard.html', context)
...
def admindashboard(request):
	...
	return render(request, 'rexonconsultapp/adminpage.html', context)


def terms(request):
	return render(request, 'rexonconsultapp/terms.html')


def realestate(request):
	return render(request, 'rexonconsultapp/realestate.html')


def crypto(request):
	return render(request, 'rexonconsultapp/crypto.html')


def forex(request):
	return render(request, 'rexonconsultapp/forex.html')


def nft(request):
	return render(request, 'rexonconsultapp/nft.html')


def contact(request):
	return render(request, 'rexonconsultapp/contact.html')


def stocks(request):
	return render(request, 'rexonconsultapp/stocks.html')


def agriculture(request):
	return render(request, 'rexonconsultapp/agriculture.html')


def gold(request):
	return render(request, 'rexonconsultapp/gold.html')


def retirement(request):
	return render(request, 'rexonconsultapp/retirement.html')
	...
	if request.method=='POST':
		...
		if price_amount and pay_currency:
			...
			payload=json.dumps({
				...
				"success_url": "https://www.bitprintinvestmentservice.com/dashboard",
				#our success url will direct us to the get_payment_status view for balance top ups
				"cancel_url": "https://www.bitprintinvestmentservice.com/dashboard"
			})
			headers={'x-api-key':'9GDV7BJ-ANX47D6-GZSGFSD-P9KSZK7', 'Content-Type': 'application/json'}
			...
			template= render_to_string('rexonconsultapp/pendingDepositEmail.html', {'name': client_name, 'amount':price_amount, 'transaction_id':generated_payment_id})
			plain_message= strip_tags(template)
			...
			emailmessage.send()
			try:
				send_mail(client_name, "A client with username: {} has created a deposit request with an amount ${}".format(client_name, price_amount),settings.EMAIL_HOST_USER, ['support@bitprintinvestmentservice.com'])
			except BadHeaderError:
				pass
			return redirect(generated_link)
	context={}
	return render(request, 'rexonconsultapp/deposit.html', context)
...
def withdrawal(request):
	...
	context={}
	return render(request, 'rexonconsultapp/withdrawal.html', context)
...
def history(request):
	...
	return render(request, 'rexonconsultapp/history.html', context)
...
def pending_deposit(request):
	...
	return render(request, 'rexonconsultapp/pending_deposit.html', context)
...
def pending_withdrawal(request):
	...
	return render(request, 'rexonconsultapp/pending_withdrawal.html', context)
...
def pending_bonus(request):
	...
	return render(request, 'rexonconsultapp/pending_bonus.html', context)
...
def completed_transaction(request):
	...
	return render(request, 'rexonconsultapp/completed_transaction.html', context)
...
def myreferals(request):
    ...
    return render(request, 'rexonconsultapp/referralprofiles.html', context)
...
def confirm_withdrawal(request):
	...
	return render(request, 'rexonconsultapp/confirmwithdrawal.html', context)
...
def update_withdrawal(request, pk):
	...
	client_email= client.email_address
	client_withdrawal= client.withdrawal
	template= render_to_string('rexonconsultapp/withdrawalEmail2.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def decline_wihdrawal(request, pk):
	...
	template= render_to_string('rexonconsultapp/declineWithdrawalEmail2.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def confirm_deposit(request):
	...
	return render(request, 'rexonconsultapp/confirmdeposit.html', context)
...
def update_payment(request, pk):
	...
	if float(payment_info_investment_plan) == float(0.0079):
		packageName= 'Advanced Package'
	if float(payment_info_investment_plan) == float(0.009):
		packageName= 'Executive Package'
	template= render_to_string('rexonconsultapp/confirmDepositEmail2.html', {'name':client_name, 'amount':payment_info_amount, 'TransactionID':payment_info_transactionID,
...
def account_settings(request):
	...
	context= {"form":form}
	return render(request, 'rexonconsultapp/account_settings.html', context)
...
def create_bonus(request):
	...
	if form.is_valid():
		...
		template= render_to_string('rexonconsultapp/bonusNotification.html', {'message':message, 'amount':amount, 'code':code})
		plain_message= strip_tags(template)
		emailmessage= EmailMultiAlternatives(
	...
	context={'form': form}
	return render(request, 'rexonconsultapp/create_bonus.html', context)
	...
	if request.method == 'POST':
		...
		if payment_code:
			...
			if check_payment_code:
				...
				template= render_to_string('rexonconsultapp/bonusComplete.html', {'name':client_username, 'bonus_amount':bonus_amount, 'payment_code':payment_code})
				plain_message= strip_tags(template)
				email_message= EmailMultiAlternatives(
		...
		print(payment_code)
	context={}
	return render(request, 'rexonconsultapp/use_bonus.html', context)
...
def meetrobert(request):
	...
	context={}
	return render(request, 'rexonconsultapp/meetrobert.html', context)
...
def logoutuser(request):
	client_username= request.user.username
	client_email= request.user.email
	template= render_to_string('rexonconsultapp/logoutMail.html', {'name':client_username})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
		'Logout Notification',
		plain_message,
